...
# Usage: ./start-probook.sh [qwen|qwen36u|gemma|granite]
set -euo pipefail
...
if [ -z "$MODE" ]; then
  echo "Usage: $0 [qwen|qwen36u|gemma|granite]" >&2
  exit 1
fi
...
case "$MODE" in
  ...
  granite) start_model "Granite 4.1 8B"             "granite-4.1-8b-Q4_K_M.gguf"                                 131072  8192 ;;
  *) echo "Usage: $0 [qwen|qwen36u|gemma|granite]" >&2; exit 1 ;;
esac
