#!/usr/bin/env bash
# Installs agent provider config for ik_llama.cpp on i9-13900.
...
MODELS_JSON=$(START_SCRIPT="$START_SCRIPT" MODELS_DIR="$MODELS_DIR" OPENCODE_OUTPUT_LIMIT="$OPENCODE_OUTPUT_LIMIT" BASE_URL="$BASE_URL" python3 -c "
...
echo "Limits: context from start-i9.sh${IK_LLAMA_CTX_SIZE:+ overridden by IK_LLAMA_CTX_SIZE=$IK_LLAMA_CTX_SIZE}, output=$OPENCODE_OUTPUT_LIMIT, OpenCode compaction reserved=$OPENCODE_COMPACTION_RESERVED"
